...
tar -czf - -C dist . | ssh "${SSH_OPTS[@]}" "$TARGET" "sudo mkdir -p '$ROOT' && sudo tar -xzf - -C '$ROOT'"

echo "==> installing nginx site on port $PORT"
sed "s/__PORT__/$PORT/g" deploy/zonke.nginx.conf | ssh "${SSH_OPTS[@]}" "$TARGET" "cat > /tmp/zonke.conf"
ssh "${SSH_OPTS[@]}" "$TARGET" bash -se <<EOF
set -euo pipefail
